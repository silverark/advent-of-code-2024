...
func process(input []string) int {
	...
	for row := range input {
		for col := range input[row] {
			...
			//log.Println("Original Guard: ", originalGuard)

			guard = originalGuard
			guard.visited = make(map[[3]int]bool)
			guard.visited[[3]int{guard.row, guard.col, 'U'}] = true

			//log.Printf("Starting guard for # at Row: %v Col: %v GDirection: %v Grow: %v Gcol: %v loop: %v inGrid: %v",
			//	row, col, guard.direction, guard.row, guard.col, guard.inLoop, guard.inGrid)
